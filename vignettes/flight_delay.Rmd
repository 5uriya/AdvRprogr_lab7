...
# Build parameters
ridge_prm <- data.frame(parameter = c("formula", "data", "lambda"),
                  class = c("formula", "data.frame", "numeric"),
                  label = c("formula", "data", "lambda"))

# Build tune grid
ridge_grid <- function(x, y, len = NULL, search = "grid"){
    lambda_grid <- data.frame(lambda = seq(0, 1, by = 0.25))
}

# Build model fit
ridge_fit <- function(x, y, wts, param, lev, last, weights, classProbs, ...){
    df <- if(is.data.frame(x)){
        x
    } else{
        as.data.frame(x)
        # Need to add dependent variable from formula
        df$arr_delay <- y
        
        # Build independent variables from formula
    output <- ridgereg$new(arr_delay ~ ., data = df, lambda = ridge_prm$lambda)
